...
func  (g *GoodsServer)GoodsList(ctx context.Context, req *proto.GoodsFilterRequest) (*proto.GoodsListResponse, error){
	//goodlistresponse:=&proto.GoodsListResponse{}
	...
	//var count int64
	//localDB.Count(&count)
	//goodlistresponse.Total=int32(count)
	...
	localDB := global.DB.Model(model.Goods{})
	if req.KeyWords != "" {
		fmt.Println(req.KeyWords,"keyword")
	...
	result, err := global.EsClient.Search().Index("goods").Query(q).From(int(req.Pages)).Size(int(req.PagePerNums)).Do(context.Background())
	if err != nil {
		return nil, err
	}
	...
	for _, value := range result.Hits.Hits {
		goods := model.EsGoods{}
		fmt.Println(value.Source,"value.Source",q)
